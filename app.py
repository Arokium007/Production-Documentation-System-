...
def _scrape_image_urls(query, num_results=10):
    """Scrape image URLs using DDG (primary) with Bing fallback."""
    urls = []
    ...
    for attempt in range(2):
        try:
            with DDGS() as ddgs:
                results = list(ddgs.images(query, max_results=num_results + 5))
                for r in results:
                    u = r.get('image', '')
                    if u and not u.endswith(('.svg', '.gif')) and u not in urls:
                        urls.append(u)
    ...
    # Strategy 2: Bing HTML scrape
    try:
        sess = requests.Session()
        sess.headers.update({
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
        })
        sess.get('https://www.bing.com/', timeout=5)
        resp = sess.get(f'https://www.bing.com/images/search?q={requests.utils.quote(query)}&form=HDRSC2', timeout=10)
        ...
        for pat in patterns:
            ...
            for u in found:
                ...
                if u not in urls and not u.endswith(('.svg', '.gif')):
                    urls.append(u)
                if len(urls) >= num_results:
                    return urls[:num_results]
    except Exception:
...
@app.route('/api/product/<int:product_id>/search-images', methods=['POST'])
def api_search_product_images(product_id):
    """AI-powered product image search using header information."""
    product = Product.query.get_or_404(product_id)
    
    try:
        pis = product.pis_data or {}
        header = pis.get('header_info', {})
        product_name = header.get('product_name', product.model_name or '')
        model_number = header.get('model_number', '')
        brand = header.get('brand', '')
        
        if not product_name and not model_number:
            return jsonify({"error": "No product name or model number available"}), 400
        
        # Step 1: AI query optimization
        context = f"Product: {product_name}, Model: {model_number}, Brand: {brand}"
        prompt = f"""Generate the best image search query to find product photos for:
{context}

Rules:
- Return ONLY the search query, nothing else
- Always include the model number or part number if available
- Keep to 3-7 words
- Optimize for finding the actual product image (not logos or unrelated items)
- If the brand is generic or unknown, focus on the product name and model"""
        
        try:
            model = genai.GenerativeModel('gemini-2.0-flash')
            resp = model.generate_content(prompt)
            ai_query = resp.text.strip().strip('"').strip("'")
            if len(ai_query) < 3 or len(ai_query) > 80:
                ai_query = f"{model_number} {product_name}".strip()
        except Exception:
            ai_query = f"{model_number} {product_name}".strip()
        
        print(f"🔍 Image search query: '{ai_query}' for product #{product_id}")
        
        # Step 2: Scrape image URLs
        image_urls = _scrape_image_urls(ai_query, 10)
        
        if not image_urls:
            # Fallback: try simple query
            fallback_query = f"{model_number} {product_name} {brand}".strip()
            print(f"🔄 Fallback query: '{fallback_query}'")
            image_urls = _scrape_image_urls(fallback_query, 10)
        
        if not image_urls:
            return jsonify({"error": "No images found for this product", "query": ai_query}), 404
        ...
        for url in image_urls:
            try:
                ...
                if img.width < 50 or img.height < 50:
                    continue
                img = img.convert('RGB')
                candidates.append({'image': img, 'bytes': r.content, 'url': url})
                if len(candidates) >= 8:
                    break
            except Exception:
                continue
        ...
        if not candidates:
            return jsonify({"error": "Could not download any valid images", "query": ai_query}), 404
        
        # Step 4: AI ranking (optional — try, fallback to first 5)
        ...
        try:
            ...
            for i, c in enumerate(candidates):
                buf = BytesIO()
                c['image'].save(buf, format='JPEG', quality=70)
                buf.seek(0)
                vision_parts.append(f"Image {i}:")
                vision_parts.append({'mime_type': 'image/jpeg', 'data': buf.getvalue()})
            
            vision_parts.append(f"""Pick the top {min(5, len(candidates))} images that best show the actual product "{product_name} {model_number}".
Return ONLY a JSON array of indices, e.g. [2, 0, 4, 1, 3].
Be lenient — watermarks and busy backgrounds are OK. Only reject completely unrelated images.""")
            
            model = genai.GenerativeModel('gemini-2.0-flash')
            vision_resp = model.generate_content(vision_parts)
            indices_text = vision_resp.text.strip()
            # Extract JSON array
            match = re.search(r'\[[\d\s,]+\]', indices_text)
            if match:
                parsed = json.loads(match.group())
                valid = [i for i in parsed if 0 <= i < len(candidates)]
                if valid:
                    selected_indices = valid[:5]
        except Exception as e:
            print(f"⚠️ Vision ranking failed, using first candidates: {e}")
        
        # Step 5: Save selected images
        saved_paths = []
        for idx in selected_indices:
            c = candidates[idx]
            fname = f"search_{product.id}_{uuid.uuid4().hex[:8]}.jpg"
            save_path = os.path.join(app.config['UPLOAD_FOLDER'], fname)
            c['image'].save(save_path, 'JPEG', quality=85)
            db_path = f"uploads/{fname}"
            saved_paths.append(db_path)
        
        # Step 6: Save to product (first = main if empty, rest = additional)
        ...
        print(f"✅ Saved {len(saved_paths)} images for product #{product_id}")
        
        return jsonify({
            "status": "success",
            "query": ai_query,
            "count": len(saved_paths),
            "images": [url_for('static', filename=p) for p in saved_paths]
        })
